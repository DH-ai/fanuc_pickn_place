...
class DeltoGripper:
    ...
    def connect(self):
        ...
        try:
            ...
            print(f"[OK] Connected to gripper at {self.ip_address}:{self.port}")
    ...
    def disconnect(self):
        if self.socket:
            print(f"Socket: {self.socket}")
            print("[OK] Disconnecting from gripper")    
            try:
                self.socket.close()
                return 1
            except Exception:
...
if __name__ == "__main__":
    gripper = DeltoGripper(ip_address="192.168.0.30", port =502)
    ...
    if gripper.connect():
        try:
            ...
            print(f"Success: {gripper.start_hold_loop(HOME)}")
            gripper.disconnect()
            # time.sleep(2)
            # gripper.connect()
            exit()
